#include <chrono>
#include <exception>
#include <judemq/judemq.h>
#include <judemq/address.h>
...
See judemq::Address C++ documentation for more details)")
...
- local_sn - if True then this command will be unavailable if the JudeMQ object was not constructed
...
This is a shortcut for `judemq.send(msg.conn, command, *args)`; use the full version if you need
extra send functionality.)")
        .def("request", [](Message& m, std::string command, JudeMQ::ReplyCallback callback, py::args args) {
...
This is a shortcut for `judemq.request(msg.conn, command, on_reply, *args)`; use the full version if you need
extra send functionality.)")
...
Unlike Message, the value returned here may outlive the command callback (as long as the JudeMQ
instance is still alive).)")
    ...
    py::class_<Message::DeferredSend>(msg, "DeferredSend")
        ...
        .def("request", [](Message::DeferredSend& d, std::string command, JudeMQ::ReplyCallback callback, py::args args) {
    ...
    py::class_<CatHelper>(mod, "Category",
            "Helper class to add in registering category commands, returned from JudeMQ.add_category(...)")
                    ...
                    cat.add_request_command(name, [handler](Message& msg) {
                        std::vector<std::string> result;
                        {
                            ...
                            try {
                                ...
                                msg.judemq.log(LogLevel::warn, __FILE__, __LINE__,
...
callback itself.)")
    ...
    py::class_<JudeMQ> judemq{mod, "JudeMQ"};
    judemq
        .def(py::init([](
                        py::bytes pubkey,
                        py::bytes privkey,
                        bool sn,
                        JudeMQ::SNRemoteAddress sn_lookup,
                        std::optional<LogLevel> log_level) {
            return std::make_unique<JudeMQ>(pubkey, privkey, sn, std::move(sn_lookup),
                    log_level ? JudeMQ::Logger{stderr_logger{}} : nullptr,
...
- log_level the initial log level; defaults to warn.  The log level can be changed later by calling
  ...
  Python's GIL.  In the future a Python callback may be supported for log messages.
        ...
        .def_readwrite("handshake_time", &JudeMQ::HANDSHAKE_TIME,
                "How long to wait for handshaking to complete on new connections before timing out.")
        .def_readwrite("ephemeral_routing_id", &JudeMQ::EPHEMERAL_ROUTING_ID,
...
Defaults to a large number.  If changing then this must be set before start() is called.)")
        .def_readwrite("reconnect_interval", &JudeMQ::RECONNECT_INTERVAL,
                "Minimum time to wait before attempting to reconnect a failed connection.")
        .def_readwrite("reconnect_interval_max", &JudeMQ::RECONNECT_INTERVAL_MAX,
...
exponential backoff will be used, up to at most this interval between reconnection attempts.)")
        .def_readwrite("close_linger", &JudeMQ::CLOSE_LINGER,
                "How long (at most) to wait for connections to close cleanly when closing.")
        .def_readwrite("connection_check_interval", &JudeMQ::CONN_CHECK_INTERVAL,
...
start().)")
        .def_readwrite("connection_heartbeat", &JudeMQ::CONN_HEARTBEAT,
...
Defaults to 15s)")
        .def_readwrite("connection_heartbeat_timeout", &JudeMQ::CONN_HEARTBEAT_TIMEOUT,
...
connections.)")
        .def_readwrite("startup_umask", &JudeMQ::STARTUP_UMASK,
...
call.)")
        .def_property_readonly("pubkey",
                ...
                "Accesses this JudeMQ's x25519 public key, as bytes.")
        .def_property_readonly("privkey",
                [](const JudeMQ& self) {
                    auto& priv = self.get_privkey();
                    return py::bytes(priv.data(), priv.size());
                },
                "Accesses this JudeMQ's x25519 private key, as bytes.")
        .def("start", &JudeMQ::start, R"(Starts the JudeMQ object.
...
- on_bind a callback to invoke when the port has been successfully opened or failed to open, called
  ...
  directly from the proxy thread and so should be fast and non-blocking.
        ...
        .def("add_tagged_thread", [](JudeMQ& self, std::string name, std::function<void()> start) {
...
- start - an optional callback to invoke from the thread as soon as JudeMQ itself starts up (i.e.
...
task to the tagged thread.
        ...
        .def("set_general_threads", &JudeMQ::set_general_threads, "threads"_a,
...
Changing this also affects the default value of set_batch_threads() and set_reply_threads().)")
        .def("set_reply_threads", &JudeMQ::set_reply_threads, "threads"_a,
...
tasks currently being processed.)")
        ...
        .def("set_batched_threads", &JudeMQ::set_batch_threads,
...
are currently processing batch jobs.)")
        ...
        .def("add_timer", py::overload_cast<
                ...
                bool,
                std::optional<TaggedThreadID>>(&JudeMQ::add_timer),
...
- thread - a TaggedThreadID specifying a tagged thread (created with `add_tagged_thread`) in which
  the timer should run.  If unspecified then the timer runs in the general batch job queue.)")
        .def("cancel_timer", &JudeMQ::cancel_timer, R"(Cancels a timer previously added with add_timer().
...
It is permitted to call this with the same TimerID multiple times; subsequent calls have no effect.)")
        .def("job", &JudeMQ::job, "job"_a, py::kw_only(), "thread"_a = std::nullopt,
                R"(Queues a job to be run as an JudeMQ job.

This submits a callback to be invoked by JudeMQ.  The job can either be scheduled with general batch
jobs or can be directed to a specific tagged thread (created with `add_tagged_thread`).)")
        .def("add_category", &JudeMQ::add_category,
...
available).
        ...
        .def("add_command_alias", &JudeMQ::add_command_alias,
                "from"_a, "to"_a,
...
"dog" category rather than the "cat" category.)")
        .def("connect_remote", [](JudeMQ& self,
                    const address& remote,
                    JudeMQ::ConnectSuccess on_success,
                    JudeMQ::ConnectFailure on_failure,
        ...
        kwonly,
        "timeout"_a = judemq::REMOTE_CONNECT_TIMEOUT,
...
`ephemeral_routing_id` and `timeout` allowing overriding the defaults (judemq.EPHEMERAL_ROUTING_ID
...
that arrive through this connection.
        ...
        .def("connect_remote", [](JudeMQ& self,
            ...
            self.connect_remote(
                    ...
                    judemq::connect_option::timeout{timeout},
                    connect_option::ephemeral_routing_id{ephemeral_routing_id.value_or(self.EPHEMERAL_ROUTING_ID)},
                    auth_level
                    );
...
- ephemeral_routing_id - if set, override the default JudeMQ.EPHEMERAL_ROUTING_ID for this
  connection.
...
send/request methods to send to the SN by pubkey.
        ...
        .def("connect_inproc", &JudeMQ::connect_inproc<>,
                "on_success"_a, "on_failure"_a,
                R"(Establish a connection to ourself.

Connects to the built-in in-process listening socket of this JudeMQ server for local communication.
...
the JudeMQ object.  The connection works whether or not there are any accessible external listeners.

Also note that incoming inproc requests are unauthenticated: that is, they will always have
admin-level access.
)")
        .def("disconnect", &JudeMQ::disconnect,
...
the background).)")
        ...
        .def("send", [](JudeMQ& self, std::variant<ConnectionID, py::bytes> conn,
                    std::string command,
            ...
            if (!request) {
                ...
                auto reply_cb = [reply_rawptr = on_reply.release(), fail_rawptr = on_reply_failure.release()]
                    (bool success, std::vector<std::string> data) {
                        ...
                        // the callback.  However judemq invokes this callback exactly once so we
...
- keep_alive - if specified and set to a datetime.timedelta then the given value will be used for
  ...
  if the messages uses an existing incoming connection.
        ...
        "Convenience shortcut for judemq.send(..., request=True)")
        .def("inject_task", &JudeMQ::inject_task,
...
external command processing to be combined with judemq task scheduling.

For example, jude-core uses this to handle RPC requests coming in over HTTP as if they were incoming
JudeMQ RPC requests, with the same scheduling and queuing of requests applied to both HTTP and
JudeMQ requests so that both are treated fairly in terms of processing priority.)")
        ;
    ...
    judemq.def("request_future", [](py::handle self, py::args args, py::kwargs kwargs) {
